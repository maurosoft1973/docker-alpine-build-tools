#!/bin/bash
# Description: Script for alpine build tools container
...
do
    case $arg in
        ...
        echo -e "  -wd=|--www_data -> ${WWW_DATA} (directory of source file)"
        exit 0
        ;;
    esac
done
...
docker run -dit --name ${CONTAINER} -v ${WWW_DATA}:/var/www:rw -e LC_ALL=${LC_ALL} -e TIMEZONE=${TIMEZONE} ${IMAGE}:${IMAGE_TAG}
